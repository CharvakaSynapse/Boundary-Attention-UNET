...
from evaluate import StandaloneEvaluator, predict_and_visualize

def main():
    # Train the model
    model, best_miou = train_model()
    
    print(f"\nTraining completed!")
    print(f"Best mIoU achieved: {best_miou:.4f}")
    print(f"Model uses E(x) - edge detection on skip connections")
    ...
    model.load_state_dict(torch.load('boundary_aware_attention_unet.pth'))
